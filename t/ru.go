...
/giveaway_1000: как только кто-либо нажмёт 'Получить' 1000 сатоши будут переведены кликеру.
	...
	COINFLIPHELP: `Запускает честную лотерею подбрасывания монетки с указанным количеством участников. Все платят одинаковое количество, равное стоимости входа. Победитель получает всё. Токены перемещаются только в тот момент, когда запускается лотерея.
...
/paywall_withdraw просто опустошает весь ваш paywall.link баланс и переводит его в баланс @{{ .BotName }}.
    `,
	PAYWALLERROR:   "<b>[paywall]</b> Ошибка: {{.Err}}",
...
/sats4ads_broadcast_1000 вещает сообщение. Последняя цифра равна максимуму сатоши, которые будут потрачены. Более дешёвые подписчики рекламы предпочтительны. Эта команда должна вызываться в ответ на другое сообщение, содержание которого будет использовано в качестве текста рекламы.
	...
	SATS4ADSNOMESSAGE: `<b>[sats4ads]</b> Используйте эту команду как ответ на предыдущее сообщение. Сообщение, на которое вы отвечаете, будет использовано в качестве контента для широковещательной рассылки.`,
	SATS4ADSBROADCAST: `<b>[sats4ads]</b> {{if .NSent}}Сообщение отправлено {{.NSent}} раз{{s .NSent}} с полной стоимостью {{.Sats}} сат ({{dollar .Sats}}).{{else}}Не могу найти подписчиков с подходящими параметрами. /sats4ads_rates{{end}}`,
	SATS4ADSPRICETABLE: `<b>[sats4ads]</b> Количество пользователей в каждом диапазоне цены.
{{range .Rates}}<code>{{.UpToRate}} мсат</code>: <i>{{.NUsers}} пользователей{{s .NUsers}}</i>
{{else}}
