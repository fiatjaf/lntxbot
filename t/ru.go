...
`,
	LNURLPAYPROMPT: `<code>{{.Domain}}</code> ожидает {{if .FixedAmount}}<i>{{.FixedAmount | printf "%g"}} сат</i>{{else}}количество между <i>{{.Min | printf "%g"}}</i> и <i>{{.Max | printf "%g"}} сат</i>{{end}} для оплаты:
...
<b>транзакция</b>: <i>{{.Hash}}</i> /tx{{.HashFirstChars}}
	...
	PAIDMESSAGE: `Оплачено <b>{{printf "%g" .Sats}} сат</b> (+ {{.Fee}} комиссия). 
...
<code>/reveal 5c0b2rh4x</code> создаёт приглашение открыть скрытое сообщение 5c0b2rh4x, если оно существует.
	...
	BITFLASHCONFIRM:      `<b>[bitflash]</b> Вы подтверждаете запрос транзакции Bitflash на <b>{{.BTCAmount}} BTC</b> на адрес <code>{{.Address}}</code>? Вы заплатите <b>{{printf "%g" .Sats}}</b>.`,
...
{{end}}
    `,
	BITFLASHHELP: `
<a href="https://bitflash.club/">Bitflash</a> это сервис, который осуществляет дешёвые ончейн транзакции при получении платежей Lightning. Это возможно, поскольку он объединяет несколько транзакций Lightning и затем делает ончейн транзакцию после достижения заданного объёма.
...
/sats4ads_broadcast_1000 вещает сообщение. Последняя цифра равна максимуму сатоши, которые будут потрачены. Более дешёвые подписчики рекламы предпочтительны. Эта команда должна вызываться в ответ на другое сообщение, содержание которого будет использовано в качестве текста рекламы.
    `,
	SATS4ADSTOGGLE:    `<b>[sats4ads]</b> {{if .On}}Смотреть рекламу и получать {{printf "%g" .Sats}} сат за символ.{{else}}Вы больше не увидите рекламы.{{end}}`,
...
Каждое сообщение стоит такую цену <i>за символ</i> + <code>1 сат</code> за каждого пользователя.
    `,
	SATS4ADSADFOOTER: `[sats4ads: {{printf "%g" .Sats}} сат]`,
...
Заплатить счёт выше?
	...
	BALANCEMSG: `
<b>Полный баланс</b>: {{printf "%g" .Sats}} сат ({{dollar .Sats}})
<b>Доступный баланс</b>: {{printf "%g" .Sats}} сат ({{dollar .Usable}})
<b>Всего получено</b>: {{printf "%g" .Received}} сат
<b>Всего отправлено</b>: {{printf "%g" .Sent}} сат
<b>Всего комиссий оплачено</b>: {{printf "%g" .Fees}} сат
...
{{range .Balances}}<code>{{.Tag}}</code>: <i>{{printf "%g" .Balance}} сат</i>  ({{dollar .Balance}})
{{else}}
