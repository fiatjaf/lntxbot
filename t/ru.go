...
`,
	LNURLPAYPROMPT: `<code>{{.Domain}}</code> ожидает {{if .FixedAmount}}<i>{{.FixedAmount | printf "%.3f"}} сат</i>{{else}}количество между <i>{{.Min | printf "%.3f"}}</i> и <i>{{.Max | printf "%.3f"}} сат</i>{{end}} для оплаты:
...
/fundbtc_1000000 создаёт ордер для перевода 0.01000000 BTC с ончейн адреса на баланс бота.
    `,
	BITCLOUDSHELP: `
<a href="https://bitclouds.sh">bitclouds.sh</a> это программируемая платформа VPS, специализированная на Биткоин-сервисах. Вы можете получить обычный VPS, предназначенный для Bitcoin Core или инстансы c-lightning за 66 сат/ч. Не существует более дешёвого сервиса!

/bitclouds отразит статус активных хостов.
/bitclouds_create отобразит доступные опции создания хоста.
<code>/bitclouds topup &lt;sats&gt;</code> пополнит счёт хоста или позволит выбрать конкретный.
...
♠️ /poker -- играйте в lightning-poker.com автоматически оплачивая buy-in и сохраняя баланс единнобразно, через ваш бот. /help_poker
